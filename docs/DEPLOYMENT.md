...
这些任务里还可能包含**目标表已变更**而被暂停的任务：它们仍计入待同步（状态接口的 `parked` 计数），
但不会写入任何表，直到在 Lark 检查页对该组执行“同步重试”，把它重新指向该组当前已确认的表。
...
本次升级开始，每个排队任务会记录提交时的目标表指纹。升级**之前**排队（或提交时不带指纹）的任务取不到
当初确认的表，会以 `target_changed` 暂停并计入状态接口的 `parked`，系统不会自动重试：在 Lark 检查页
对该组执行一次“同步重试”，即可把这些任务重新指向该组当前的表。升级后请按此处理一次 `parked > 0` 的组。
