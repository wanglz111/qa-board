...
提示词是**整份**发给 AI 的：`GET /api/ai-prompts` 直接返回整个 markdown，导入页的复制 / 下载按钮给的也是整份，所以 `## 第二部分`「格式速查」和 `## 第三部分`「常见错误写法」AI 一定会读到——这两节必须按「AI 会读到」来写，不得出现「可选字段只能省略」这类反向暗示，必要时要显式声明「本节不是对第一部分的放宽」。
