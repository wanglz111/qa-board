...
9. **（2026-09-18，`22b621a`）附录 G 的 O1/O2/O3/O10 已修**：幂等签名加 `savedGroupId`；`submitting` 改成在飞计数器；`LegacyHistory` 的复测按钮加 `disabled`（同时关掉 O3b 的入口，并让 spec「二、F」第 4 条的收窄在 UI 上不可达）；保存的 catch 按「提交失败」与「提交后读取失败」分别措辞，并把预留退租挪到提交后读取链完成时（否则读取失败后重试会多写一行，`a9b227a`）。O1 的症状描述已更正为 409 循环。仍开放：O3b 的剩余形态、O8、O9、O11、O12。详见 spec 附录 G 的状态段。
