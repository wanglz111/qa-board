...
5. AI 推断不出这一轮属于哪个分层：如果本轮是冒烟轮，复制提示词时再加一句「**`执行分层` 整列填 `Smoke`**」。
...
| 列 | 怎么填 |
| --- | --- |
| 执行结果 | 只能填 `通过` / `不通过` / `未执行`，或**留空**。原文写 ✅ / Pass / OK / 成功 → `通过`；❌ / Fail / NG / 失败 → `不通过`；原文写未测 / 未执行 / TBD / `-` → `未执行`；我要求先不导入结论的用例 → **留空**（留空 ≠ `未执行`，见下）。**不要写「阻塞」**：系统不接受这个值，写了整份会被拒绝。 |
| 实测过程 | 把原文里我写下的观测**搬运**过来，一字不改：选择器、实测值、报错原文、口径标签、复现路径。**禁止**写「验证通过」「符合预期」「正常」「功能可用」这类判断词；原文没有观测就留空。 |

- **留空 = 只建用例，不建执行记录**（界面按未测显示，Lark 里不出现这一行）；这与填 `未执行` **不同**——`未执行` 是一条真实结果，会建执行记录，并在 Lark 里显示「未执行」。所以我让你留空的，就是我不想现在导入结论的那些。
...
| 执行分层 | 只填英文枚举 `Smoke` / `Core` / `Regression`；这一轮是冒烟轮就整列 `Smoke`，拿不准就填 `Core`。**逐行必须有值，且必须是英文枚举**（写「冒烟层」不合格） |
| 前置条件 | 逐字保留；原文没有就留空 |
| 测试数据 | 逐字保留；原文没有就留空 |
...
> 本节是给人看的补充说明，**不是对第一部分的放宽**：若你正在生成 CSV，读到本节也不能据此把 `优先级`、`执行分层` 留空，或删掉这两列。完整规则见 [IMPORT-FORMAT.md](IMPORT-FORMAT.md)。

- 只接受 `.csv` / `.json` / `.md`，UTF-8，单文件 ≤10MB、≤5000 条；导入器层面必填的只有 `用例编号` 与 `用例标题`；但本提示词额外要求 `优先级`、`执行分层` 两列**逐行必填**——漏写不会报错，只会静默落成空值（Lark 里全部显示 P2），所以不能据此省列。
- `优先级` 写成 `P0/P1/P2` 之外的值，写进 Lark 时会静默兜底成默认 `P2`；`执行分层` **导入端不校验枚举**，写「冒烟层」这类中文会原样进库（只有带图用例包那条路会拒绝），结果和 casebook 包里的 `Smoke` 变成两种写法。
...
- **同一个字段的多个别名不要同时出现**：`执行结果` 的四个别名（`result`、`执行结果`、`实测结果`、`本轮实测结果`）只能保留一列，`实测过程` 的四个别名（`evidence`、`实测过程`、`过程记录`、`实测说明`）也只能保留一列；同时出现会被判字段歧义、整份拒绝。
...
| 同一个字段的多个别名同时出现（如 `执行结果` 与 `实测结果`／`本轮实测结果`／`result`，或 `实测过程` 与 `过程记录`／`实测说明`／`evidence`） | 同一个字段的多个别名不要同时出现，同时出现会被判字段歧义、整份拒绝；每个字段只保留你在表头里用的那一列 |
