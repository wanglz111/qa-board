...
it("refuses a retest while a save is in flight and offers it again once the save lands", async () => {
  ...
  // not dispatch on a disabled element, so a 「the click did nothing」 assertion
  // here could never fail and would only look like coverage.
...
it("does not offer a retry that a committed reservation can never accept", async () => {
  const reserved: Attempt = {
    id: "attempt-retest",
    label: "B-001-R0918-a1b2c3-01",
    sequence: 2,
  ...
  const commitReserved = vi
    .fn()
    .mockResolvedValueOnce({ ...reserved, state: "committed", result: "通过" })
    // The edited payload mints a key no row carries, so the route refuses the
    // attempt it already committed.
    .mockRejectedValueOnce(new ApiError(409, "Attempt is already committed"));
  const submit = vi.fn<(groupId: string, code: string, payload: SubmitPayload) => Promise<Attempt>>();
  let reads = 0;
  const loadAttempts = vi.fn<(groupId: string, code: string) => Promise<Attempt[]>>(async () => {
    reads += 1;
  ...
  // The commit landed; only the read-back failed, so the reservation is still in
  // hand and the form still holds its fields.
  expect(await screen.findByText(/结果已保存到本地，但执行记录读取失败/)).toBeVisible();

  // Editing the payload and pressing save is exactly the gesture 「可重试」 asks
  // for, and it can never succeed here: the reservation is committed server-side,
  // so the new key matches nothing and the route answers 409. Telling the operator
  // to retry would send them round that loop for good.
  await userEvent.type(screen.getByLabelText("控制台输出"), "改了说明");
  await userEvent.click(screen.getByRole("button", { name: /保存结果/ }));

  expect(await screen.findByText(/已经提交过，这次修改没有保存/)).toBeVisible();
  expect(screen.queryByText(/可重试/)).not.toBeInTheDocument();
  expect(commitReserved).toHaveBeenCalledTimes(2);
  expect(submit).not.toHaveBeenCalled();
});
